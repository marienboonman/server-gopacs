...
async def handle_flex_request(flex_request_root: etree._Element):
    ...
    print('============')
    await send_signed_message(inner_bytes, token,recipient_domain,"AGR")
...
async def uftp_endpoint(request: Request, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        # -------------------------------------------------------
        # 1️⃣ Stuur meteen OK terug
        # -------------------------------------------------------
        # LET OP: géén return_ok() meer, maar gewoon return hier onderaan
        # De verwerking gaat in een background task!
        # -------------------------------------------------------

        # Voeg de daadwerkelijke verwerking toe als background task:
...
async def uftp_endpoint(request: Request):
    ...
    try:
        ...
        #etree.tostring(inner_xml_bytes, pretty_print=True)
        print('ONTVANGEN BERICHT')
        print(inner_xml_bytes.decode('utf-8'))
        print('=============')
        # -------------------------------------------------------
...
###OLD CODE BLOCK DELETE AFTER TEST
'''
...
async def get_public_key(sender_role: str, sender_domain: str) -> bytes:
    ...
    async with httpx.AsyncClient() as client:
        ...
        data = r.json()
        public_key_b64 = data["publicKey"]
        public_key_b64 = "yqHFTQJ5cVJY7NG58iGfGmrKnYUyHDHNpIu0evuS8S4="
        return base64.b64decode(public_key_b64)
